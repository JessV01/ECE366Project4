# ECE 366 Project 4
# Created by: Jessica Vargas, Vaishnavi Medikundam
# The program takes in MIPS instruction memory in hex and returns data

import math

def MIPSsim(HexInstr, BinInstr, Part2, Multi, Pipe, finished):
   
    size_of_mem = 1024                      
    if(Multi):
        print("-------------------------\nBeginning MIPS simulation (multicycle)...\n-------------------------")
    if(Pipe):
        print("-------------------------\nBeginning MIPS simulation (pipeline)...\n-------------------------")
    
    Reg = [0,0,0,0,0,0,0,0]                # the $0-$7 registers initialized to 0
    Mem = [0 for i in range(size_of_mem)]   # creates memory array initialized to 0's

    blkA = [-1,-1]
    blkB = [-1,-1,-1,-1]
    tagC_prev = -1
    num_blks = 8
    num_ways = 2
    num_set = int(num_blks/num_ways)
    blkD = [-1 for i in range(num_set)]

    PC = 0
    DIC = 0
    multi_cycles = 0 # total number of cycles for multicycle CPU
    pipe_cycles = 0  # total number of cycles for pipeplined CPU
    threeCyc = 0    # number of instructions that take...3 cycles   
    fourCyc = 0     # number of instructions that take...4 cycles
    fiveCyc = 0     # number of instructions that take...5 cycles
    hazz = 0             # checks on the number of hazards when goes through
    hitsA = 0        #number of total hits
    hitsB = 0        #number of total hits
    hitsC = 0        #number of total hits
    hitsD = 0        #number of total hits
    missesA = 0      #number of total misses
    missesB = 0      #number of total misses
    missesC = 0      #number of total misses
    missesD = 0      #number of total misses
    
    end = False
    while(not(end)):
        DIC += 1
        fetch = BinInstr[PC]
        fetch2 = HexInstr[PC]
        size = len(BinInstr)
        if((PC+1) < size):
            get1 = BinInstr[PC+1]
        if((PC+2) < size):
            get2 = BinInstr[PC+2]
            
        if(fetch[0:32] == '00010000000000001111111111111111'):
            threeCyc += 1
            multi_cycles += 3
            if(PC == 0):
                pipe_cycles += 5
            else:
                pipe_cycles += 1
            if(Part2):
                if (Multi):
                    print("*CYCLE "+str(multi_cycles-2)+"*")
                    print("PC = "+str(PC*4)) 
                    print(str(fetch2[0:16])+" --> beq $0,$0,-1")
                    print("Takes 3 cycles...")
                    print("Deadloop Reached")
                if (Pipe):
                    print("PC = "+str(PC*4)) 
                    print(str(fetch2[0:16])+" --> beq $0,$0,-1")
                    print("Deadloop Reached")
            end = True
            
        elif(fetch[0:6] == '000000'): # R-type instructions
            Rs = int(fetch[6:11],2)
            Rt = int(fetch[11:16],2)
            Rd = int(fetch[16:21],2)
            fourCyc += 1 # all R-type instructions take 4 cycles in multicycle CPU
            multi_cycles += 4
            if(PC == 0):
                pipe_cycles += 5
            else:
                pipe_cycles += 1
    
            if(fetch[26:32] == '100000'): # 'add'
                PC += 1
                Reg[Rd] = Reg[Rs] + Reg[Rt]
                if(Part2):
                    if (Multi):
                        print("*CYCLE "+str(multi_cycles-3)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> add $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))
                        print("Takes 4 cycles...\n")
                    if (Pipe):
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> add $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))
                        
            elif(fetch[26:32] == '100010'): # 'sub'
                PC += 1
                Reg[Rd] = Reg[Rs] - Reg[Rt]
                if(Part2):
                    if (Multi): 
                        print("*CYCLE "+str(multi_cycles-3)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> sub $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))
                        print("Takes 4 cycles...\n")
                    if (Pipe):
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> sub $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))
                        
            elif(fetch[26:32] == '100110'): # 'xor'
                PC += 1
                Reg[Rd] = Reg[Rs] ^ Reg[Rt]
                if(Part2):
                    if (Multi): 
                        print("*CYCLE "+str(multi_cycles-3)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> xor $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))
                        print("Takes 4 cycles...\n")
                    if (Pipe):
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> xor $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))
                        
            elif(fetch[26:32] == '101010'): # 'slt'
                PC += 1
                if(Reg[Rs] < Reg[Rt]):
                    Reg[Rd] = 1
                else:
                    Reg[Rd] = 0
                if(Part2):
                    if (Multi): 
                        print("*CYCLE "+str(multi_cycles-3)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> slt $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))
                        print("Takes 4 cycles...\n")
                    if (Pipe): 
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> slt $"+str(Rd)+",$"+str(Rs)+",$"+str(Rt))

            if(Pipe): #where forwarding hazards are checked
                if((fetch[0:6] != '000100') & (fetch[0:6] != '000101') & (fetch[0:6] != '100011')):
                    if(get1[0:6] == '000000'):
                        Rs1 = int(get1[6:11],2)
                        Rt1 = int(get1[11:16],2)
                        if(((Rs1 == Rd) | (Rt1 == Rd)) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                            print("Hazard detected...")
                            print("Hazard solved by forwarding with no delay\n")
                        else:
                            if(get2[0:6] == '000000'):
                                Rs2 = int(get2[6:11],2)
                                Rt2 = int(get2[11:16],2)
                                if(((Rs2 == Rd) | (Rt2 == Rd)) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                            else:
                                Rs2 = int(get2[6:11],2)
                                if((Rs2 == Rd) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                            
                    else:
                        Rs1 = int(get1[6:11],2)
                        if((Rs1 == Rd) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                            print("Hazard detected...")
                            print("Hazard solved by forwarding with no delay\n")
                        else:
                            if(get2[0:6] == '000000'):
                                Rs2 = int(get2[6:11],2)
                                Rt2 = int(get2[11:16],2)
                                if(((Rs2 == Rd) | (Rt2 == Rd)) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                            else:
                                Rs2 = int(get2[6:11],2)
                                if((Rs2 == Rd) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                
        else:   #I-type instructions
            Rs = int(fetch[6:11],2)
            Rt = int(fetch[11:16],2)
            if(fetch[16] == '0'):
                Imm = int(fetch[16:32],2)
            else:
                Imm = -(65535 -int(fetch[16:32],2)+1)
            if(PC == 0):
                pipe_cycles += 5
            else:
                pipe_cycles += 1

            if(fetch[0:6] == '001000'): # 'addi'
                fourCyc += 1
                multi_cycles += 4
                PC += 1
                Reg[Rt] = Reg[Rs] + Imm
                if(Part2):
                    if (Multi): 
                        print("*CYCLE "+str(multi_cycles-3)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> addi $"+str(Rt)+",$"+str(Rs)+","+str(Imm))
                        print("Takes 4 cycles...\n")
                    if (Pipe): 
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> addi $"+str(Rt)+",$"+str(Rs)+","+str(Imm))

            elif(fetch[0:6] == '000100'): # 'beq'
                threeCyc += 1
                multi_cycles += 3
                stall = False
                flush = False
                get = BinInstr[PC-1]
                if(get[0:6] == '000000'): #checking for stall
                    prev_Rd = int(get[16:21],2)
                    if((prev_Rd == Rs) | (prev_Rd == Rt)):
                        pipe_cycles += 1
                        hazz +=1
                        stall = True
                if(Reg[Rs] == Reg[Rt]):
                    PC = PC + Imm + 1
                    pipe_cycles += 1 #branch flush
                    hazz += 1
                    flush = True
                else:
                    PC += 1
                if(Part2):
                    if (Multi): 
                        print("*CYCLE "+str(multi_cycles-2)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> beq $"+str(Rs)+",$"+str(Rt)+","+str(Imm))
                        print("Takes 3 cycles...\n")
                    if (Pipe):
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> beq $"+str(Rs)+",$"+str(Rt)+","+str(Imm))
                        if((stall == True) | (flush == True)):
                            print("Hazard detected...")
                            if((stall == True) & (flush == True)):
                                print("Hazard solved with a stall and flushing\n")
                            elif(stall == True):
                                print("Hazard solved with a stall\n")
                            elif(flush == True):
                                print("Hazard solved with flushing\n")
                        else:
                            print("No hazard detected\n")
                        
            elif(fetch[0:6] == '000101'): # 'bne'
                threeCyc += 1
                multi_cycles += 3
                stall = False
                flush = False
                get = BinInstr[PC-1]
                if(get[0:6] == '000000'): #checking for stall
                    prev_Rd = int(get[16:21],2)
                    if((prev_Rd == Rs) | (prev_Rd == Rt)):
                        pipe_cycles += 1
                        hazz +=1
                        stall = True
                if(Reg[Rs] != Reg[Rt]):
                    PC = PC + Imm + 1 
                    pipe_cycles += 1 #branch flush
                    hazz += 1
                    flush = True
                else:
                    PC += 1
                if(Part2):
                    if (Multi): 
                        print("*CYCLE "+str(multi_cycles-2)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> bne $"+str(Rs)+",$"+str(Rt)+","+str(Imm))
                        print("Takes 3 cycles...\n")
                    if (Pipe): 
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> bne $"+str(Rs)+",$"+str(Rt)+","+str(Imm))
                        if((stall == True) | (flush == True)):
                            print("Hazard detected...")
                            if((stall == True) & (flush == True)):
                                print("Hazard solved with a stall and flushing\n")
                            elif(stall == True):
                                print("Hazard solved with a stall\n")
                            elif(flush == True):
                                print("Hazard solved with flushing\n")
                        else:
                            print("No hazard detected\n")
                            
            elif(fetch[0:6] == '100011'): # 'lw'
                fiveCyc += 1
                multi_cycles += 5
                stall = False
                PC += 1
                Reg[Rt] = Mem[((Reg[Rs] + Imm)-8192)]
                get = BinInstr[PC+1]
                if(get[0:6] == '000000'): #checking for stall
                    prev_Rs = int(get[6:11],2)
                    prev_Rt = int(get[11:16],2)
                    if((prev_Rs == Rs) | (prev_Rs == Rt) | (prev_Rt == Rs) | (prev_Rt == Rt)):
                        pipe_cycles += 1
                        hazz += 1
                        stall = True
                else:
                    prev_Rt = int(get[11:16],2)
                    if((prev_Rt == Rs) | (prev_Rt == Rt)):
                        pipe_cycles += 1
                if(Part2):
                    if (Multi):
                        print("*CYCLE "+str(multi_cycles-4)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> lw $"+str(Rt)+","+str(Imm)+"($"+str(Rs)+")")
                        print("Takes 5 cycles...")
                    if (Pipe):
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> lw $"+str(Rt)+","+str(Imm)+"($"+str(Rs)+")")
                        if(stall == True):
                            print("Hazard detected...")
                            print("Hazard solved with a stall")
                        else:
                            print("No hazard detected")
                            
                    print("*Cache Access Log*:")
                    #-------------------------------------------
                    print("   a.")
                    hit = False
                    addr = format(((Reg[Rs] + Imm)-8192), "016b") #16 bit address
                    a = int(addr[11:12],2) #blk number
                    tag = int(addr[0:11],2)
                    vbit = 0
                    if(blkA[a] == -1):
                        hit = False
                        blkA[a] = tag
                        missesA += 1
                        vbit = 0
                    else:
                        if(blkA[a] == tag):
                            hit = True
                            hitsA += 1
                        else:
                            hit = False
                            blkA[a] = tag
                            missesA += 1
                        vbit = 1
                    hitrate = (hitsA/(hitsA+missesA))*100
                    hitrateA = int(math.ceil(hitrate))
                    print("     Memory address to access = "+str(hex(Reg[Rs] + Imm)))
                    print("     Block of cache accessed = "+str(a))
                    print("     Valid bit & tag info: Valid Bit = "+str(vbit)+" Tag Info = "+str(hex(blkA[a]+8192)))
                    if(hit):
                        print("     Hit or Miss? = Hit")
                        print("     Cache update: Cache remains the same\n")
                    else:
                        print("     Hit or Miss? = Miss")
                        print("     Cache update: Block "+str(a)+"'s new tag is "+str(hex(tag))+"\n")
                    #---------------------------------------------
                    print("   b.")
                    hit = False
                    addr = format(((Reg[Rs] + Imm)-8192), "016b") #16 bit address
                    b = int(addr[11:13],2) #blk number
                    tag = int(addr[0:11],2)
                    vbit = 0
                    if(blkB[b] == -1):
                        hit = False
                        blkB[b] = tag
                        missesB += 1
                        vbit = 0
                    else:
                        if(blkB[b] == tag):
                            hit = True
                            hitsB += 1
                        else:
                            hit = False
                            blkB[b] = tag
                            missesB += 1
                        vbit = 1
                    hitrate = (hitsB/(hitsB+missesB))*100
                    hitrateB = int(math.ceil(hitrate))
                    print("     Memory address to access = "+str(hex(Reg[Rs] + Imm)))
                    print("     Block of cache accessed = "+str(b))
                    print("     Valid bit & tag info: Valid Bit = "+str(vbit)+" Tag Info = "+str(hex(blkB[b]+8192)))
                    if(hit):
                        print("     Hit or Miss? = Hit")
                        print("     Cache update: Cache remains the same\n")
                    else:
                        print("     Hit or Miss? = Miss")
                        print("     Cache update: Block "+str(b)+"'s new tag is "+str(hex(tag))+"\n")
                    #-------------------------------------------------
                    print("   c.")
                    tag = int(addr[0:13],2)
                    vbit = 0
                    if(tagC_prev == -1):
                        hit = False
                        missesC += 1
                        vbit = 0
                    else:
                        if(tagC_prev == tag):
                            hit = True
                            hitsC += 1
                            vbit = 1
                        else:
                            hit = False
                            missesC += 1
                            vbit = 0
                    tagC_prev = tag
                    hitrate = (hitsC/(hitsC+missesC))*100
                    hitrateC = int(hitrate)
                    print("     Memory address to access = "+str(hex(Reg[Rs] + Imm)))
                    print("     Set/Block of cache accessed = None")
                    print("     Valid bit & tag info: Valid Bit = "+str(vbit)+" Tag Info = "+str(hex(tag+8192)))
                    if(hit):
                        print("     Hit or Miss? = Hit")
                    else:
                        print("     Hit or Miss? = Miss")
                    print("     No cache update needed \n")
                    #-------------------------------------------------
                    print("   d.")
                    hit = False
                    addr = format(((Reg[Rs] + Imm)-8192), "016b") #16 bit address
                    d = int(addr[11:13],2) #"blk" index
                    setidx = int(addr[9:11],2) #set index
                    settag = int(addr[0:9],2)
                    tag = int(addr[0:11],2)
                    vbit = 0
                    if(blkD[d] == -1):
                        hit = False
                        blkD[d] = tag
                        missesD += 1
                        vbit = 0
                    else:
                        if(blkD[d] == tag):
                            hit = True
                            hitsD += 1
                            vbit = 1
                        else:
                            hit = False
                            blkD[d] = tag
                            missesD += 1
                            vbit = 0
                    hitrate = (hitsD/(hitsD+missesD))*100
                    hitrateD = int(hitrate)
                    print("     Memory address to access = "+str(hex(Reg[Rs] + Imm)))
                    print("     Set of cache accessed = "+str(setidx))
                    print("     Valid bit & tag info: Valid Bit = "+str(vbit)+" Tag Info = "+str(hex(settag+8192)))
                    if(hit):
                        print("     Hit or Miss? = Hit")
                        print("     Cache update: Cache remains the same\n")
                    else:
                        print("     Hit or Miss? = Miss")
                        print("     Cache update: One of the ways in set "+str(setidx)+" has a new tag which is "+str(hex(settag))+"\n")
                        
            elif(fetch[0:6] == '101011'): # 'sw'
                fourCyc += 1
                multi_cycles += 4
                PC += 1
                Mem[((Reg[Rs] + Imm)-8192)] = Reg[Rt]
                if(Part2):
                    if (Multi): 
                        print("*CYCLE "+str(multi_cycles-3)+"*")
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> sw $"+str(Rt)+","+str(Imm)+"($"+str(Rs)+")")
                        print("Takes 4 cycles...\n")
                    if (Pipe):
                        print("PC = "+str((PC-1)*4)) 
                        print(str(fetch2[0:16])+" --> sw $"+str(Rt)+","+str(Imm)+"($"+str(Rs)+")")

            if(Pipe): #where forwarding hazards are checked
                if((fetch[0:6] != '000100') & (fetch[0:6] != '000101') & (fetch[0:6] != '100011')):
                    if(get1[0:6] == '000000'):
                        Rs1 = int(get1[6:11],2)
                        Rt1 = int(get1[11:16],2)
                        if((Rs1 == Rt) | (Rt1 == Rt)):
                            print("Hazard detected...")
                            print("Hazard solved by forwarding with no delay\n")
                        else:
                            if(get2[0:6] == '000000'):
                                Rs2 = int(get2[6:11],2)
                                Rt2 = int(get2[11:16],2)
                                if(((Rs2 == Rt) | (Rt2 == Rt)) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                            else:
                                Rs2 = int(get2[6:11],2)
                                if((Rs2 == Rt) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                            
                    else:
                        Rs1 = int(get1[6:11],2)
                        if((Rs1 == Rt) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                            print("Hazard detected...")
                            print("Hazard solved by forwarding with no delay\n")
                        else:
                            if(get2[0:6] == '000000'):
                                Rs2 = int(get2[6:11],2)
                                Rt2 = int(get2[11:16],2)
                                if(((Rs2 == Rt) | (Rt2 == Rt)) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                            else:
                                Rs2 = int(get2[6:11],2)
                                if((Rs2 == Rt) & ((fetch[0:6] != '000100') | (fetch[0:6] != '000101'))):
                                    print("Hazard detected...")
                                    print("Hazard solved by forwarding with no delay\n")
                                else:
                                    print("No hazard detected\n")
                        
    print("-------------------------\nSimulation done...\n-------------------------")
    if(finished):
        print("*Results*:")
        print("Dynamic Instruction Count: " + str(DIC))
        print("PC: " + str(PC*4))
        print("Registers: ")
        print("   $1 = " + str(Reg[1]))
        print("   $2 = " + str(Reg[2]))
        print("   $3 = " + str(Reg[3]))
        print("   $4 = " + str(Reg[4]))
        print("   $5 = " + str(Reg[5]))
        print("   $6 = " + str(Reg[6]))
        print("   $7 = " + str(Reg[7]))
        if(Part2):
            print("Overall Hit Rate:")
            print("   a. "+str(hitrateA)+"%")
            print("   b. "+str(hitrateB)+"%")
            print("   c. "+str(hitrateC)+"%")
            print("   d. "+str(hitrateD)+"%")
            print("For Multicycle CPU:")
            print("   Total Cycle Count = "+str(multi_cycles))
            print("   Number of 3-cycle Instructions: "+str(threeCyc))
            print("   Number of 4-cycle Instructions: "+str(fourCyc))
            print("   Number of 5-cycle Instructions: "+str(fiveCyc))
            print("For Pipelined CPU:")
            print("   Total Cycle Count = "+str(pipe_cycles))
            print("   Breakdown of hazards = "+str(hazz))

def main():
    i_memfile = open("i_mem.txt", "r")
    HexInstr = [] #array for instructions in hex
    BinInstr = [] #array for instructions in binary
    Part2 = True if int(input("1 = Part 1\n2 = Part 1&2&3\n>>")) == 2 else False
    for l in i_memfile: #reading each line in txt file
        if (l == "\n" or l[0] == '#'): #skips over empty lines and comments
            continue
        l = l.replace('\n','')
        HexInstr.append(l)
        l = format(int(l,16),"032b")
        BinInstr.append(l)
    Multi = True 
    Pipe = False
    finished = False
    MIPSsim(HexInstr, BinInstr, Part2, Multi, Pipe, finished) #MIPSsim function
    Multi = False
    Pipe = True
    finished = True
    MIPSsim(HexInstr, BinInstr, Part2, Multi, Pipe, finished)

if __name__ == "__main__": 
    main()
